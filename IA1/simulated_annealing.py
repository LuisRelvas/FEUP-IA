...
def add_library_to_signing_random(state,days,libraries):
    ...
    if new_state_add_librarySigning.library_signing == [] and len(new_state_add_librarySigning.library_explored) != len(libraries) :
        unexplored_and_unsigned_libraries = [lib for lib in libraries if not utils.check_libraryExplored(new_state_add_librarySigning,lib) and not utils.check_librarySigning(new_state_add_librarySigning,lib)]
        if unexplored_and_unsigned_libraries:
            library = random.choice([lib for lib in libraries if not utils.check_libraryExplored(new_state_add_librarySigning,lib) and not utils.check_librarySigning(new_state_add_librarySigning,lib) and libraries != []])
...
def calculate_initial_temperature(state, days, libraries):
    ...
    if not neighbors: 
        print("No Neighbors Found. Restarting...")
        return None
    sample_deltas = []
    ...
    if not sample_deltas:
        print("No sample deltas found. Restarting...")
        return None
...
def simulated_annealing(initial_state, days, libraries,cooling_rate,max_iterations,stop_temperature):
    ...
    while current_temp is None: 
        new_random_solution = utils.random_solution(libraries, current_state.book_scores, days)
        current_temp = calculate_initial_temperature(new_random_solution, days, libraries)
    ...
    while True:
        ...
        if neighbors: 
            ...
            if delta_value > 0: 
                ...
                print("Current Value: ", current_state.get_value(days)[0])
                print("---------------------------------------------------------------")
            else: 
                try:
